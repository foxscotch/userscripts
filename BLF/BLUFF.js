...
User.prototype.updateInfo = function (doc) {
  if (
    doc.querySelector('a[title*="Personal Message"]').children[0].alt ==
    "Online"
  ) {
    this.online = true;
  } else {
...
User.prototype.getProfileInfo = function (callback) {
  ...
  request.open(
    "get",
    "http://forum.blockland.us/index.php?action=profile;u=" + user.memberId
  );
  request.send();
};
  ...
  if (pt.profile) {
    if (pt.profile.summary) {
      ...
      var params = getQueryParams(
        document
          .querySelectorAll('td.windowbg2[colspan="2"] > a')[0]
          .href.split("?")[1]
      );
