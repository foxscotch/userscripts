...
function addTextBox() {
  ...
  breaks[breaks.length - 2].parentElement.insertBefore(
    container,
...
function getQuote(link) {
  ...
  xhr.open(
    "GET",
    mainUrl +
      `index.php?action=quotefast;quote=${link.dataset.messageId};sesc=${sessId};xml`
  );
  xhr.send();
}

function addQuoteButtons() {
  var quoteDestinations = document.querySelectorAll(
    'td[height="20"]:not([width])'
  );
  ...
  for (var i = 0; i < quoteDestinations.length; i++) {
    ...
    var msgId = quoteDestinations[i].parentElement
      .querySelector("div")
      .id.split("_")[1];
